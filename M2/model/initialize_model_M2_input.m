function m = initialize_model_M2_input(m,varargin)
...
%% inputs
% m.u = [];
m.info.u{1} = {'k_FC_synt'               'hepatic de novo FC synthesis '                      ''}; % gene
m.info.u{2} = {'k_FC_cat'                'free cholesterol catabolism'                        ''}; % gene 
m.info.u{3} = {'k_FC_conv_CE_cyt'        'hepatic conversion of FC to CE (cytosol)'           ''};
m.info.u{4} = {'k_FC_conv_CE_ER'         'hepatic conversion of FC to CE (ER)'                ''};
m.info.u{5} = {'k_CE_conv_FC_cyt'        'hepatic conversion of CE to FC (cytosol)'           ''};
m.info.u{6} = {'k_CE_conv_FC_ER'         'hepatic conversion of CE to FC (ER)'                ''};

m.info.u{7} = {'k_TG_dnl_synt_cyt'       'hepatic de novo TG synthesis (cytosol)'              ''}; % gene 
m.info.u{8} = {'k_TG_cat_cyt'            'hepatic TG catabolism (cytosol)'                     ''}; % gene
m.info.u{9} = {'k_TG_ER_to_cyt'          'hepatic non de novo TG transport (ER to cytosol)'    ''};
m.info.u{10} = {'k_TG_cyt_to_ER'         'hepatic non de novo TG transport (cytosol to ER)'    ''};

m.info.u{11} = {'k_TG_lip_per'           'peripheral TG uptake (lipolysis)'                     ''};
m.info.u{12} = {'k_upt_per'              'peripheral TG uptake (whole particle uptake)'         ''};
m.info.u{13} = {'k_TG_lip_hep'           'hepatic TG uptake (lipolysis)'                        ''}; % gene
m.info.u{14} = {'k_upt_hep'              'hepatic TG uptake (whole particle uptake)'            ''}; % gene

m.info.u{15} = {'k_VLDL_TG_synt'         'hepatic non de novo VLDL-TG secretion'             ''}; % gene
m.info.u{16} = {'k_VLDL_CE_synt'         'hepatic VLDL-C secretion'                          ''}; % gene 

m.info.u{17} = {'k_CE_for_HDL'           'peripheral C efflux to HDL'                        ''};
m.info.u{18} = {'k_CE_upt_HDL'           'hepatic HDL-C uptake'                              ''};

m.info.u{19} = {'k_FFA_upt'              'hepatic FFA uptake'                                ''}; % gene 
m.info.u{20} = {'k_FFA_secr'             'net FFA secretion to plasma'                       ''};

m.info.u{21} = {'k_ApoB_prod'            'hepatic ApoB secretion'                            ''}; % gene 
m.info.u{22} = {'k_TG_dnl_synt_ER'       'hepatic de novo TG synthesis (cytosol)'            ''}; % gene 
...
m.eq.y = {
            {'k_dud' 'k_dud'}
%           {'hep_TG_ndnl_cyt + hep_TG_ndnl_ER + hep_TG_dnl_cyt + hep_TG_dnl_ER' 'hep_TG'}
