...
/*-----------------------------------------------------------------*/
/* Added by Joep Vanlier */

int IDASetMaxTime( void *ida_mem, double maxTime )
{
    IDAMem IDA_mem;
            ...
            realtype t0, N_Vector yy0, N_Vector yp0)
{
  ...
  /* Set max integration time Joep Vanlier */
  
  IDA_mem->tMax = DBL_MAX;
             ...
             N_Vector yret, N_Vector ypret, int itask)
{
  ...
  time_t tStart, tEnd; //Added by Joep Vanlier
  ...
  time( &tStart );  /* Added by J. Vanlier */

  loop {

      time( &tEnd ); /* Added by J. Vanlier */
      if ( difftime( tEnd, tStart ) > IDA_mem->tMax ) { /* Added by Joep Vanlier */
        fprintf(stderr, "Simulation time exceeded: t=%e", tn );
