function freeze_colormap(varargin)
% freeze_colormap  Lock colors of plot, enabling multiple colormaps per figure. (v2.3)
%
%   Problem: There is only one colormap per figure. This function provides
%       an easy solution when plots using different colomaps are desired 
%       in the same figure.
%
%   freeze_colormap freezes the colors of graphics objects in the current axis so 
%       that subsequent changes to the colormap (or caxis) will not change the
%       colors of these objects. freeze_colormap works on any graphics object 
...
%       based on the colormap active at the time freeze_colormap is called.
%
%   The original indexed color data is saved, and can be restored using
%       unfreeze_colormap, making the plot once again subject to the colormap and
%       caxis.
%
%
%   Usage:
%       freeze_colormap        applies to all objects in current axis (gca),
%       freeze_colormap(axh)   same, but works on axis axh.
%
%   Example:
%       subplot(2,1,1); imagesc(X); colormap hot; freeze_colormap
%       subplot(2,1,2); imagesc(Y); colormap hsv; freeze_colormap etc...
%
%       Note: colorbars must also be frozen. Due to Matlab 'improvements' this can
%				no longer be done with freeze_colormap. Instead, please
...
%
%   Side effect on render mode: freeze_colormap does not work with the painters
%       renderer, because Matlab doesn't support rgb color data in
%       painters mode. If the current renderer is painters, freeze_colormap
%       changes it to zbuffer. This may have unexpected effects on other aspects
%	      of your plots.
%
%       See also unfreeze_colormap, freeze_colormap_pub.html, cbfreeze.
%
%
...
%   freeze_colormap('nancolor',[r g b]) or freeze_colormap(axh,'nancolor',[r g b]),
...
appdatacode = 'JRI__freeze_colormapData';
...
for hh = cdatah',
    ...
    %save original indexed data for use with unfreeze_colormap
    siz = size(cdata);
    ...
    if strcmp(scalemode,'scaled'),
        ...
        idx = cdata;
        %10/8/09 in case direct data is non-int (e.g. image;freeze_colormap)
...
% checkArgs  Validate input arguments to freeze_colormap
...
if mod(nargs,2),
    h = args{1};
    if ~ishandle(h),
        error('JRI:freeze_colormap:checkArgs:invalidHandle',...
    ...
    if strcmp(get(h,'Tag'),'Colorbar'),
      if ~exist('cbfreeze.m'),
        warning('JRI:freeze_colormap:checkArgs:cannotFreezeColorbar',...
            ['You seem to be attempting to freeze a colorbar. This no longer'...
            'works. Please read the help for freeze_colormap for the solution.'])
      else
        cbfreeze(h);
        return
      end
...
if nargs == 2,
    if strcmpi(args{end-1},'nancolor'),
        ...
        if ~all(size(nancolor)==[1 3]),
            error('JRI:freeze_colormap:checkArgs:badColorArgument',...
                'nancolor must be [r g b] vector');
        end
        nancolor(nancolor>1) = 1; nancolor(nancolor<0) = 0;
    else
        error('JRI:freeze_colormap:checkArgs:unrecognizedOption',...
